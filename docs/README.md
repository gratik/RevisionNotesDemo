# Documentation Index

## Metadata
- Owner: Maintainers
- Last updated: February 15, 2026
- Prerequisites: None
- Related examples: docs/Learning-Path.md


Start here:

- [Learning Path](Learning-Path.md)

## Foundations

- [OOP Principles](OOP-Principles.md) - Beginner - 60 min
- [Core C#](Core-CSharp.md) - Beginner - 90 min
- [DotNet Concepts](DotNet-Concepts.md) - Beginner - 60 min
- [Modern C#](Modern-CSharp.md) - Intermediate - 75 min
- [LINQ Queries](LINQ-Queries.md) - Beginner - 60 min
- [Memory Management](Memory-Management.md) - Intermediate - 60 min
- [Async Multithreading](Async-Multithreading.md) - Intermediate - 90 min
- [Advanced C#](Advanced-CSharp.md) - Advanced - 75 min

## Backend and API

- [Configuration](Configuration.md) - Beginner - 45 min
- [Web API and MVC](Web-API-MVC.md) - Intermediate - 90 min
- [API Documentation](API-Documentation.md) - Intermediate - 45 min
- [Data Access](Data-Access.md) - Intermediate - 75 min
- [SQL Server Docs Index](Data-Access/SqlServer/README.md) - Advanced - 20 min
- [SQL Server Queries and Operations](Data-Access/SqlServer/SqlServerQueriesAndOperations.md) - Advanced - 20 min
- [Table Design Fundamentals](Data-Access/SqlServer/TableDesignFundamentals.md) - Intermediate - 15 min
- [Normalization and Denormalization Patterns](Data-Access/SqlServer/NormalizationAndDenormalizationPatterns.md) - Intermediate - 15 min
- [Relational Data Modeling Patterns](Data-Access/SqlServer/RelationalDataModelingPatterns.md) - Intermediate - 15 min
- [Partitioning and Data Lifecycle Patterns](Data-Access/SqlServer/PartitioningAndDataLifecyclePatterns.md) - Advanced - 20 min
- [Index Architecture Patterns](Data-Access/SqlServer/IndexArchitecturePatterns.md) - Advanced - 15 min
- [Statistics and Cardinality Patterns](Data-Access/SqlServer/StatisticsAndCardinalityPatterns.md) - Advanced - 15 min
- [Execution Plan Analysis Lab](Data-Access/SqlServer/ExecutionPlanAnalysisLab.md) - Advanced - 20 min
- [Stored Procedure Design Standards](Data-Access/SqlServer/StoredProcedureDesignStandards.md) - Intermediate - 15 min
- [Concurrency Consistency Patterns](Data-Access/SqlServer/ConcurrencyConsistencyPatterns.md) - Advanced - 20 min
- [Bulk Ingestion Patterns](Data-Access/SqlServer/BulkIngestionPatterns.md) - Advanced - 15 min
- [Operational Monitoring Runbook](Data-Access/SqlServer/OperationalMonitoringRunbook.md) - Advanced - 15 min
- [SQL Security Patterns](Data-Access/SqlServer/SqlSecurityPatterns.md) - Advanced - 15 min
- [CTE vs Temp/Table Variable Patterns](Data-Access/SqlServer/CteTempTableAndTableVariablePatterns.md) - Advanced - 15 min
- [MERGE and Upsert Patterns](Data-Access/SqlServer/MergeAndUpsertPatterns.md) - Advanced - 15 min
- [SQL Graph Data Transfer Patterns](Data-Access/SqlServer/SqlGraphDataTransferPatterns.md) - Advanced - 20 min
- [Transaction and Isolation Patterns](Data-Access/SqlServer/TransactionAndIsolationPatterns.md) - Advanced - 20 min
- [Entity Framework](Entity-Framework.md) - Intermediate - 90 min
- [Security](Security.md) - Advanced - 120 min
- [Testing](Testing.md) - Intermediate - 90 min
- [Practical Patterns](Practical-Patterns.md) - Intermediate - 75 min
- [RealTime](RealTime.md) - Intermediate - 45 min
- [gRPC](gRPC.md) - Advanced - 60 min
- [Health Checks](HealthChecks.md) - Beginner - 30 min
- [Logging and Observability](Logging-Observability.md) - Intermediate - 75 min
- [End-to-End Case Study](End-to-End-Case-Study.md) - Advanced - 90 min

## Architecture and Scale

- [Design Patterns](Design-Patterns.md) - Intermediate - 120 min
- [Domain-Driven Design](Domain-Driven-Design.md) - Advanced - 90 min
- [Distributed Consistency](Distributed-Consistency.md) - Advanced - 75 min
- [Resilience](Resilience.md) - Advanced - 60 min
- [Performance](Performance.md) - Advanced - 75 min
- [Message Architecture](Message-Architecture.md) - Advanced - 75 min
- [Deployment and DevOps](Deployment-DevOps.md) - Intermediate - 75 min
- [Azure Hosting](Azure-Hosting.md) - Advanced - 90 min
- [Build Warning Triage](Build-Warning-Triage.md) - Intermediate - 25 min
- [Suppression Audit](Suppression-Audit.md) - Intermediate - 20 min
- [Front-End DotNet UI](Front-End-DotNet-UI.md) - Intermediate - 90 min
- [.NET API to React Front End](DotNet-API-React.md) - Intermediate - 50 min
- [.NET API to Vue Front End](DotNet-API-Vue.md) - Intermediate - 50 min
- [Interview Preparation](Interview-Preparation.md) - Beginner - 60 min
- [Interview Docs Hub](interview/README.md) - Beginner - 10 min
- [Interview Answer Blocks](interview/Interview-Answer-Blocks.md) - Intermediate - 35 min
- [Interview Bad vs Strong Answers](interview/Interview-Bad-vs-Strong-Answers.md) - Intermediate - 25 min
- [Interview Timed Practice Sets](interview/Interview-Timed-Practice-Sets.md) - Intermediate - 35 min
- [Interview Role Tracks](interview/Interview-Role-Tracks.md) - Intermediate - 20 min
- [Interview Common Traps](interview/Interview-Common-Traps.md) - Intermediate - 20 min
- [Interview Whiteboard Prompts](interview/Interview-Whiteboard-Prompts.md) - Advanced - 30 min
- [Interview Last-Day Revision Sheet](interview/Interview-Last-Day-Revision-Sheet.md) - Beginner - 20 min
- [Interview Behavioral-Technical Bridges](interview/Interview-Behavioral-Technical-Bridges.md) - Intermediate - 25 min
- [Interview Cheat Card](interview/Interview-Cheat-Card.md) - Beginner - 10 min
- [Interview Role Mock Scripts](interview/Interview-Role-Mock-Scripts.md) - Intermediate - 30 min
- [Interview Scoring Sheets](interview/Interview-Scoring-Sheets.md) - Intermediate - 20 min
- [Interview Mock Runbook](interview/Interview-Mock-Runbook.md) - Intermediate - 20 min
- [Interview Readiness Dashboard](interview/Interview-Readiness-Dashboard.md) - Intermediate - 15 min
- [Interview Weekly Sprint Planner](interview/Interview-Weekly-Sprint-Planner.md) - Intermediate - 20 min
- [Interview Progress Log](interview/Interview-Progress-Log.md) - Beginner - 15 min
- [Interview Question-to-Module Map](interview/Interview-Question-Module-Map.md) - Intermediate - 20 min
- [Content Coverage](Content-Coverage.md) - Intermediate - 20 min
- [Content Improvement Plan](Content-Improvement-Plan.md) - Intermediate - 20 min
- [Topic README Template](Topic-README-Template.md) - Beginner - 15 min
- [Project Structure and Inventory](Project-Structure-Inventory.md) - Beginner - 20 min
- [Runtime Section Map](Runtime-Section-Map.md) - Beginner - 10 min

## Appendices

- [Appendix: Pattern Decision Guide](Appendix-Pattern-Decision-Guide.md) - Intermediate - 30 min
- [Appendix: Quick Reference Decision Tables](Appendix-Quick-Reference.md) - Beginner - 20 min
- [Appendix: Interview Playbook](Appendix-Interview-Playbook.md) - Beginner - 30 min

---

## Interview Answer Block

- 30-second answer: This topic covers Documentation index and study planning and focuses on clear decisions, practical tradeoffs, and production-safe defaults.
- 2-minute deep dive: Start with the core problem, explain the implementation boundary, show one failure mode, and describe the mitigation or optimization strategy.
- Common follow-up: How would you apply this in a real system with constraints?
- Strong response: State assumptions, compare at least two approaches, and justify the chosen option with reliability, maintainability, and performance impact.
- Tradeoff callout: Over-engineering this area too early can increase complexity without measurable delivery or runtime benefit.

## Interview Bad vs Strong Answer

- Bad answer: "I know docs navigation and study planning and I would just follow best practices."
- Strong answer: "For docs navigation and study planning, I first define the constraints, compare two viable approaches, justify the choice with concrete tradeoffs, and describe how I would validate outcomes in production."
- Why strong wins: It demonstrates structured reasoning, context awareness, and measurable execution rather than generic statements.

## Interview Timed Drill

- Time box: 10 minutes.
- Prompt: Explain how you would apply docs navigation and study planning in a real project with one concrete constraint (scale, security, latency, or team size).
- Required outputs:
  - One design or implementation decision
  - One risk and mitigation
  - One measurable validation signal
- Self-check score (0-3 each): correctness, tradeoff clarity, communication clarity.
