...
30-second answer:
- gRPC is about contract-first RPC communication. It matters because it optimizes service-to-service communication and typed contracts.
- Use it when high-throughput internal service calls with strict schemas.

2-minute answer:
- Start with the problem gRPC solves in this module and the baseline implementation approach.
...
Bad answer:
- Defines gRPC but skips constraints, alternatives, and production impact.

Strong answer:
- Explains when to choose gRPC, what to compare it against, and how to validate it in tests/operations.
## Interview Timed Drill
- 60 seconds: define gRPC and map it to one concrete implementation in this module.
- 3 minutes: compare gRPC with an alternative, then walk through one failure mode and mitigation.