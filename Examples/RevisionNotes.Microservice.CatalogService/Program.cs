...
using RevisionNotes.Microservice.CatalogService.Security;
...
{
    options.AddPolicy("catalog.write", policy => policy.RequireAuthenticatedUser());
});

builder.Services.AddDbContext<AppDbContext>(options => options.UseInMemoryDatabase("catalog-service-db"));
...
{
    options.AddPolicy("catalog-list", policy => policy.Expire(TimeSpan.FromSeconds(20)).Tag("catalog"));
});
...
using (var scope = app.Services.CreateScope())
{
    ...
    app.Logger.LogInformation("Seeded microservice demo data");
}
