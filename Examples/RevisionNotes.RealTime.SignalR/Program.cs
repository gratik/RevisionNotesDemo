...
using RevisionNotes.RealTime.SignalR.Contracts;
using RevisionNotes.RealTime.SignalR.Infrastructure;
using RevisionNotes.RealTime.SignalR.Realtime;
using RevisionNotes.RealTime.SignalR.Security;
...
{
    options.AddPolicy("realtime.user", policy => policy.RequireAuthenticatedUser());
});
...
builder.Services.AddHealthChecks()
    ...
    .AddCheck<RealtimeHealthCheck>("realtime", tags: ["ready"]);
...
{
    var expectedUser = config["DemoCredentials:Username"] ?? "signalr-user";
...
app.MapGet("/api/chat/history", async (IMessageHistoryStore historyStore, CancellationToken cancellationToken) =>
    Results.Ok(await historyStore.GetRecentAsync(cancellationToken)))
    .RequireAuthorization("realtime.user");
